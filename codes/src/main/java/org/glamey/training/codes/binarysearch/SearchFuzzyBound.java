...
public class SearchFuzzyBound {
    ...
    public static void main(String[] args) {
        ...
        System.out.println(getLastLessThan(nums, 6, 0, nums.length - 1));
    }
     ...
     */
    private static int getLastLessThan(int[] nums, int target, int left, int right) {
        if (left > right) {
            return -1;
        }
        ...
        if (nums[mid] > target) {
            return getLastLessThan(nums, target, left, mid - 1);
        } else {
            return getLastLessThan(nums, target, mid + 1, right);
        }
    }
